...
1.  **Functional Changes (User Facing)**
...
2.  **Structural & Logic Changes (Developer Facing)**
...
3. ever use a period at the end of the header description.
...
BREAKING CHANGE: the `login` parameter now requires an object containing provider details.
```
...
```text
refactor: simplify generic adapter logic

- Remove redundant interface checks
- Consolidate error handling into a centralized utility
- Improve type safety for external inputs
```
